...
from base_model import resnet152
from seg_opr.seg_oprs import AttentionRefinement, FeatureFusion

class Network_Res152(nn.Module):
    def __init__(self, out_planes, is_training, BN2D = BatchNorm2d):
        super(Network_Res152, self).__init__()
        self.layers = []
        self.is_training = is_training
       
        conv_channel = 128
        # use base myModels of resnet 101 from resnet.py
        self.context = resnet152(pretrained_model=None, norm_layer=BN2D, bn_eps=config.bn_eps, bn_momentum=config.bn_momentum, deep_stem=False, stem_width=64)
